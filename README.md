...
-- ========== FIXED SECTION ==========
local SHOULDER_OFFSET = 1.75;
local BIND_NAME = "CustomShiftLockCam";
local oldAR;
local shiftLockActive = false;

local function faceCam(h, hrp, v)
	-- ALWAYS clean up the previous binding first, no matter what
	local unbound = pcall(function()
		RS:UnbindFromRenderStep(BIND_NAME);
	end);

	if v then
		-- Store original AutoRotate only if we're freshly enabling
		...
		RS:BindToRenderStep(BIND_NAME, Enum.RenderPriority.Camera.Value + 1, function()
			local cam = Wk.CurrentCamera;
			if not (hrp and hrp.Parent and cam) then
				return;
			end;

			-- 1) rotate character to face the camera's flat look direction
			...
			-- 2) shift camera to the RIGHT so character appears on the LEFT
			cam.CFrame = cam.CFrame + (cam.CFrame.RightVector * SHOULDER_OFFSET);
		end);
	else
		shiftLockActive = false;

		-- Force AutoRotate back to true (safe default) regardless of oldAR
		-- This is the key fix: some games have AutoRotate=true by default,
		-- and relying on oldAR can fail if it was never captured or was false
		h.AutoRotate = true;

		-- Clear any angular velocity that our CFrame forcing may have caused
		if hrp and hrp.Parent then
			pcall(function()
				hrp.RotVelocity = Vector3.zero;
			end);
			-- Also clear AssemblyAngularVelocity for newer engine versions
			pcall(function()
				hrp.AssemblyAngularVelocity = Vector3.zero;
			end);
		end;

		-- Reset the mouse behavior immediately
		if UIS.MouseEnabled then
			UIS.MouseBehavior = Enum.MouseBehavior.Default;
		end;

		-- Give the engine a frame to reclaim camera control, then ensure AutoRotate sticks
		task.defer(function()
...
sc.Scale = 0.9;
sc.Parent = btn;
(Tw:Create(sc, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
	Scale = 1
})):Play();
...
local function pulse()
	...
	(Tw:Create(p, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Size = UDim2.fromOffset(m, m),
...
btn.MouseButton1Up:Connect(function()
	(Tw:Create(btn, tP, {
		Size = over and UDim2.fromOffset(70, 70) or UDim2.fromOffset(64, 64)
	})):Play();
	(Tw:Create(strk, tP, {
		Thickness = 2,
		Transparency = 0.25
...
local function shn()
	...
	g.Parent = b;
	b.Parent = s;
	local tw = Tw:Create(b, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local function apply(v)
	...
	faceCam(h, r, v);
	(Tw:Create(strk, tT, {
		Color = v and Color3.fromRGB(0, 200, 255) or Color3.fromRGB(0, 140, 255),
		Thickness = v and 3 or 2
	})):Play();
	(Tw:Create(btn, tT, {
		BackgroundColor3 = v and Color3.fromRGB(38, 48, 60) or Color3.fromRGB(34, 34, 38)
	})):Play();
	(Tw:Create(icn, tT, {
		ImageColor3 = v and Color3.fromRGB(0, 200, 255) or Color3.fromRGB(255, 255, 255)
	})):Play();
...
xBtn.Activated:Connect(function()
	...
	(Tw:Create(sc, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
		Scale = 0.8
	})):Play();
	(Tw:Create(btn, TweenInfo.new(0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
		BackgroundTransparency = 1
	})):Play();
	(Tw:Create(strk, TweenInfo.new(0.12), {
		Transparency = 1
	})):Play();
